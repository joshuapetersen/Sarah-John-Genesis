...
class SarahReasoning:
    ...
    def solve_complex_problem(self, problem_statement):
        ...
        # Inject HLE Warnings into the prompt
        hle_warnings = ""
        if self.hle_data:
        ...
        def solve_sub(sub):
            solve_prompt = f"""
            SUB-PROBLEM: {sub}
            CONTEXT: Part of larger problem: "{problem_statement}"
            
            TASK: Provide a concise, high-probability solution or implementation step.
            """
            try:
                sol_response = self._generate_with_retry(
                    model=self.model_id,
                    contents=solve_prompt,
                    config=self.config
                )
                return sub, sol_response.text.strip()
            except Exception as e:
                return sub, f"Analysis Failed: {e}"
        ...
        try:
            ...
            # 6. Sovereign Tribunal Review (The 1-3-9 Loop)
            if self.fractal_gate:
            ...
            if self.memory:
                self.memory.log_interaction(
                    ...
                    tags=["advanced_solver", "synthesis", "tribunal_reviewed"]
                )
